...
var SOURCE_KEY = 'si_data_source_v1'; // 'demo' | 'import' | 'supabase'
var LAST_IMPORT_KEY = 'si_last_import_meta_v1';
...
function clearImportStore() {
  ...
  _importCache = null;
  _wilayahCache = null;
  try { localStorage.setItem(SOURCE_KEY, _sbClient ? 'supabase' : 'demo'); } catch (e) {}
}

function saveLastImportMeta(meta) {
  try { localStorage.setItem(LAST_IMPORT_KEY, JSON.stringify(meta)); } catch (e) {}
}

function loadLastImportMeta() {
  try {
    var raw = localStorage.getItem(LAST_IMPORT_KEY);
    return raw ? JSON.parse(raw) : null;
  } catch (e) { return null; }
}

/** Active feed label for UI. Data path: import local > supabase > demo seed. */
function getDataSource() {
  var saved = null;
  try { saved = localStorage.getItem(SOURCE_KEY); } catch (e) {}
  if (saved === 'import' && loadImport()) return 'import';
  if (_sbClient) return 'supabase';
...
function setDataSource(src) {
  if (src === 'supabase') {
    if (!_sbClient) throw new Error('Supabase belum terhubung.');
    try { localStorage.setItem(SOURCE_KEY, 'supabase'); } catch (e) {}
    return getDataSource();
  }
  if (src !== 'demo' && src !== 'import') throw new Error('source harus demo, import, atau supabase');
  if (src === 'import' && !loadImport()) throw new Error('Belum ada data import lokal. Upload CSV dulu.');
...
function applyModeUI() {
  ...
  var isReal = src === 'import' || src === 'supabase' || !!_sbClient;

  // ponytail: Mode Real copy only for admin-import account
  if (isReal && !isAdmin) {
  ...
  if (src === 'import') {
    ...
    if (text) text.textContent = 'Mode Real (CSV lokal) — data dari file di browser ini.';
    if (card) {
      card.classList.add('show');
      if (title) title.textContent = 'Mode Real';
      if (desc) desc.textContent = 'Import lokal aktif di browser ini.';
      if (link) { link.textContent = 'Import Data →'; link.href = 'import-data.html'; }
    }
  } else if (src === 'supabase' || _sbClient) {
    if (banner) banner.style.display = 'flex';
    if (text) text.textContent = 'Mode Real (Supabase) — data dari database. Upload CSV di Import Data akan menulis ke DB.';
    if (card) {
      card.classList.add('show');
      if (title) title.textContent = 'Mode Real';
      if (desc) desc.textContent = 'Terhubung Supabase. Data shared antar user/device.';
      if (link) { link.textContent = 'Import Data →'; link.href = 'import-data.html'; }
    }
  } else {
    if (banner) banner.style.display = 'flex';
    ...
    if (card) {
      ...
      if (desc) desc.textContent = 'Data sample ditampilkan. Import CSV atau hubungkan Supabase.';
      if (link) { link.textContent = 'Import Data →'; link.href = 'import-data.html'; }
    }
  }
}
...
function importFromCSV(text) {
  ...
  // offline: local only — also the only data source when Mode Real local
  saveImport(payload);
  saveLastImportMeta({
    rowCount: payload.rowCount,
    schoolCount: payload.schoolCount,
...
async function upsertScoresToSupabase(flat) {
  // ponytail: replace-all so dashboard shows only this CSV (no leftover seed)
  var delScores = await _sbClient.from('monthly_scores').delete().neq('school_code', '');
  if (delScores.error) throw new Error('clear scores: ' + delScores.error.message);
  var delSchools = await _sbClient.from('schools').delete().neq('school_code', '');
  if (delSchools.error) throw new Error('clear schools: ' + delSchools.error.message);
...
async function getScoresForMonth(month) {
  ...
  try {
    ...
    if (error) throw error;
    // ponytail: never mix demo into Supabase feed — empty month stays empty
    ...
    console.warn('Supabase getScoresForMonth failed:', e.message || e);
    return []; // no demo blend when DB is the source of truth
  }
}
...
window.sbAPI = {
  ...
  // ─ CSV import (localStorage / Supabase) ─
  ...
  getImportMeta() {
    ...
    var last = loadLastImportMeta();
    if (!imp && !wil && !last && !_sbClient) return null;
    var src = getDataSource();
    return {
      rowCount: imp ? imp.rowCount : (last ? last.rowCount : 0),
      schoolCount: imp ? imp.schoolCount : (last ? last.schoolCount : 0),
      months: imp ? (imp.months || []) : (last ? (last.months || []) : []),
      importedAt: imp ? imp.importedAt : (last ? last.importedAt : null),
      active: src === 'import' || src === 'supabase',
      wilayahRows: wil ? wil.rowCount : 0,
      hasScores: !!imp || !!last || !!_sbClient,
      hasWilayah: !!wil,
      target: last ? last.target : (imp ? 'local' : (_sbClient ? 'supabase' : null)),
      source: src,
    };
  },
  hasImport: () => !!loadImport() || !!loadWilayahImport() || !!loadLastImportMeta() || !!_sbClient,
  // ─ Tahun Ajaran ─
